...
static void ADC1_RegConfig(void)
{
	...
	ADC_InitStructure.ChsNumber = ADC1_NbrOfChannel + 1;		 				
...
//	ADC_ConfigRegularChannel(ADC1, ADC_CH_16, 6, ADC1_SampleTime);	
	ADC_ConfigRegularChannel(ADC1, ADC_CH_INT_VREF, 6, ADC1_SampleTime);
...
//	ADC_SetBypassCalibration(ADC1, DISABLE);
	...
	DMA_InitStructure.BufSize 					= ADC1_SampleBufferSize + 1;					
...
static void ADC2_RegConfig(void)
{
	ADC_InitType ADC_InitStructure;
...
//	while(ADC_GetResetCalibrationStatus(ADC2));	
	/* enable Adc calibration bypass. */
//	ADC_SetBypassCalibration(ADC2, DISABLE);
...
uint16_t ADC2_GetData(void)
{
	...
	ADC_ConfigRegularChannel(ADC2, ADC2_Channel_02_PA5, 1, ADC_SAMP_TIME_55CYCLES5);
...
uint16_t ADC_GetSample(uint8_t type)
{
    ...
    switch (type)
    {
		...
		case BAT1TEMP_SAMPLE:
			temp = ADC1_SampleBuffer[4];
			break;

		case VREF_SAMPLE:
			temp = ADC1_SampleBuffer[5];
			break;
