...
static void show_addr(int argc, char **argv)
{
	...
	if(argc == 1){
		...
		if(!rt_strcmp("-help", argv[1])){
			...
			rt_kprintf("            [!note] : length must <=32.\n");
...
#define __is_print(ch)       ((unsigned int)((ch) - ' ') < 127u - ' ')
		switch(show_type)
		{
			case _HEX:
				/* 防止指针操作非法地址 */
				if(((address + length) > 0x08020000 && (address + length) < 0x20000000) || 	//非法地址区
					((address + length) > 0x20006000) || ((address + length) < 0x08000000)										
				){
			...
			case _WORD:
				if(((address + length) > 0x08020000 && (address + length) < 0x20000000) || 	
					((address + length) > 0x20006000) || ((address + length) < 0x08000000)									
				){
					rt_kprintf("operation illegal address.\r\n");
					return;
