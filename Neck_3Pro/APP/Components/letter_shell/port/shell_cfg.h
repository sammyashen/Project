 ...
 */
#define     SHELL_GET_TICK()            tiny_tick_get()//HAL_GetTick()
...
#define     SHELL_MALLOC(size)          malloc(size)//pvPortMalloc(size)

/**
 * @brief shell内存释放
 *        shell本身不需要此接口，若使用shell伴生对象，需要进行定�? */
#define     SHELL_FREE(obj)             free(obj)//vPortFree(obj)
 ...
 * @brief shell默认用户
 */
#define     SHELL_DEFAULT_USER          "breo"
